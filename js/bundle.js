...
function calc() {
  ...
  function getStaticInformation(selector, activeClass) {
    const elements = document.querySelectorAll(`${selector}`);
    elements.forEach(elem => {
      // Цей спосіб є доцільнішим, так як при йому не виникає багів з простором між кнопками
...
function slider(_ref) {
  ...
  const slides = document.querySelectorAll(slide),
    ...
    // {container}, а також це являється нашим головним слайдером
  ...
  next.addEventListener('click', () => {
    if (offset == cutNum(width) * (slides.length - 1)) {
      // offset == 650px * 3 - 1950px, mean 4 slide
      offset = 0;
    } else {
      offset += cutNum(width);
    }
